 ...
 * comment!
 */
// modification: use reference (rust orz)
...
void parse_csv(std::string filename, std::vector<Course>& courses) {
  ...
  if (!ifs.is_open()) {
    std::cout << "failed to open courses.csv" << '\n';
  }
  ...
  while(std::getline(ifs, single_line)) {
    std::vector <std::string> tokens = split(single_line, ',');
    if (tokens.size() == 3) {
...
void write_courses_offered(std::vector<Course>& all_courses) {
  ...
  if (!ofs.is_open()) {
    std::cout << "failed to open courses_offered.csv" << '\n';
  }
  ...
  // remove not offered courses
  for (auto course: courses_to_delete) {
